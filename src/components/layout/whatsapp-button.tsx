...
// glyph below is a small inline SVG (the standard Font Awesome "whatsapp"
// icon path, released under a free license).
...
// "Contact us!" label is only shown from `sm` up — on phone-width
// screens there isn't room for the label next to the icon without it
// overflowing/overlapping, so mobile just gets the icon. The ring behind
// the icon only blinks once every 20 seconds (via the `whatsapp-blink`
// keyframes in globals.css) instead of pulsing nonstop, so it stays
...
export function WhatsAppButton() {
  return (
    ...
    >
      <span className="hidden whitespace-nowrap rounded-full bg-white px-5 py-3 text-base font-semibold text-foreground shadow-md sm:inline-block">
        <strong>Contact us!</strong> Chat with us on WhatsApp
      </span>

      <span className="relative flex size-16 shrink-0 items-center justify-center rounded-full bg-[#25D366] text-white shadow-xl sm:size-27">
